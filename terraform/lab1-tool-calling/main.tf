...
# Use cloud_provider and cloud_region from core infrastructure
locals {
...
# ------------------------------------------------------
# UNIFIED RESOURCES FOR LAB1-TOOL-CALLING
# ------------------------------------------------------
...
resource "confluent_flink_statement" "email_mcp_connection" {
  organization {
  ...
  statement_name = "email-mcp-connection-create"

  statement = <<-EOT
    CREATE CONNECTION IF NOT EXISTS `${data.terraform_remote_state.core.outputs.confluent_environment_display_name}`.`${data.terraform_remote_state.core.outputs.confluent_kafka_cluster_display_name}`.`email-mcp-connection`
    WITH (
      'type' = 'MCP_SERVER',
...
# Create the Email MCP Model via Flink SQL (AWS/Bedrock)
resource "confluent_flink_statement" "email_mcp_model_aws" {
  ...
  statement_name = "email-mcp-model-create"

  statement = <<-EOT
    CREATE MODEL IF NOT EXISTS `${data.terraform_remote_state.core.outputs.confluent_environment_display_name}`.`${data.terraform_remote_state.core.outputs.confluent_kafka_cluster_display_name}`.`email_mcp_model`
    INPUT (prompt STRING)
    ...
    WITH (
      ...
      'mcp.connection' = 'email-mcp-connection'
    );
  EOT
  ...
  depends_on = [
    confluent_flink_statement.email_mcp_connection
  ]
}

# Create the Email MCP Model via Flink SQL (Azure/OpenAI)
resource "confluent_flink_statement" "email_mcp_model_azure" {
...
resource "null_resource" "generate_flink_sql_summary" {
  ...
  triggers = {
    mcp_connection = confluent_flink_statement.email_mcp_connection.id
    mcp_model_aws  = local.cloud_provider == "aws" ? confluent_flink_statement.email_mcp_model_aws[0].id : ""
    mcp_model_azure = local.cloud_provider == "azure" ? confluent_flink_statement.email_mcp_model_azure[0].id : ""
  }
  ...
  depends_on = [
    confluent_flink_statement.email_mcp_connection,
    confluent_flink_statement.email_mcp_model_aws,
    confluent_flink_statement.email_mcp_model_azure,
