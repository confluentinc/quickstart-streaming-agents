...
# ------------------------------------------------------
# LLM INFRASTRUCTURE — CONDITIONAL PER CLOUD PROVIDER
# ------------------------------------------------------

# --- AWS Bedrock Connections (only when cloud_provider == "aws") ---
...
# --- Azure OpenAI Connections (only when cloud_provider == "azure") ---
...
resource "confluent_flink_connection" "azureopenai_connection" {
  ...
  display_name = "llm-textgen-connection"
  type         = "AZUREOPENAI"
  endpoint     = "${local.azure_openai_endpoint}/openai/deployments/gpt-5-mini/chat/completions?api-version=2024-08-01-preview"
...
resource "confluent_flink_connection" "azureopenai_embedding_connection" {
  ...
  display_name = "llm-embedding-connection"
  type         = "AZUREOPENAI"
  endpoint     = "${local.azure_openai_endpoint}/openai/deployments/text-embedding-ada-002/embeddings?api-version=2024-08-01-preview"
  api_key      = var.azure_openai_api_key
...
# --- LLM Models (conditional per cloud) ---
...
# Text Generation Model — Azure (OpenAI)
...
resource "confluent_flink_statement" "llm_embedding_model_aws" {
  ...
  depends_on = [
    confluent_flink_connection.bedrock_embedding_connection
  ]
}
