...
class ConfigurationManager:
    ...
    def set_config_file_path(self, cloud_provider: str):
        ...
        self.config_file = self.root_dir / self.cloud_provider / "core" / ".setup_config.json"
        ...
        while True:
            ...
            if choice in lab_options:
                ...
                if selection in ['lab2', 'all']:
                    ...
                    self.ui.print_warning("LAB2 NOTE: You MUST run MongoDB in the same region as Confluent Cloud. Make sure that you select a MongoDB Free Tier (M0) Cluster in the same region as Confluent Cloud.")
                    self.ui.print_info("")
                    self.ui.print_warning("LAB2 NOTE: You MUST set the following values:")
                    print("")
                    print("```sh")
    ...
    def is_first_run(self) -> bool:
        ...
        core_fields = ['prefix', 'cloud_provider', 'confluent_cloud_api_key', 'confluent_cloud_api_secret']
    ...
    def save_field_incrementally(self, field: str, value: str, lab_selection: str = None):
    ...
    def save_credentials_to_tfvars(self, config: Dict[str, str]):
        ...
        try:
            ...
            core_config = {}
            core_fields = ['prefix', 'cloud_provider', 'cloud_region', 'confluent_cloud_api_key', 'confluent_cloud_api_secret', 'azure_subscription_id']
    ...
    def _write_tfvars_file(self, tfvars_file: Path, config: Dict[str, str], module_name: str):
...
'''
    ...
    def test_confluent_api_keys(self, api_key: str, api_secret: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                return True, "API keys format looks valid (network validation skipped - requests module not available)"
    ...
    def prompt_for_configuration(self, non_interactive: bool = False) -> Dict[str, str]:
        ...
        while True:
            zapier_url = self.ui.prompt("Zapier SSE Endpoint (from Zapier MCP server)", default_zapier_url)
...
class TerraformManager:
    ...
    def initialize(self, cloud_provider: str = None) -> bool:
        ...
        success, output = self.run_terraform_command(['init'], show_output=False, cloud_provider=cloud_provider)
    ...
    def plan(self, cloud_provider: str = None) -> bool:
        ...
        success, output = self.run_terraform_command(['plan'], show_output=True, cloud_provider=cloud_provider)
    ...
    def apply(self, cloud_provider: str = None) -> bool:
        ...
        success, output = self.run_terraform_command(['apply', '--auto-approve'], show_output=True, cloud_provider=cloud_provider)
...
class StreamingAgentsSetup:
    ...
    def __init__(self, args):
        ...
        self.config_manager = ConfigurationManager(self.ui, self.terraform_dir, self.root_dir)
    ...
    def deploy_core_if_needed(self, config: Dict[str, str]) -> bool:
        ...
        if not self.check_core_deployment():
            self.ui.print_error("Core deployment completed but required outputs are missing")
            self.ui.print_info("This may indicate a partial deployment failure")
            return False

        self.ui.print_success("✅ Core infrastructure deployed successfully with all required outputs")
        return True
...
'''
    ...
    def get_cloud_provider_selection(self) -> str:
        ...
        azure_valid = azure_tfvars.get('cloud_provider') == 'azure' and len(azure_tfvars) > 1
        ...
        if detected_provider:
            self.ui.print_info(f"Found existing {detected_provider.upper()} configuration")
            if self.ui.confirm(f"Continue with {detected_provider.upper()}?", default=True):
    ...
    def handle_config_and_deploy(self, message: str, **config_args) -> bool:
        ...
        if 'lab_selection' not in config_args:
            config_args['lab_selection'] = self.config_manager.prompt_for_lab_selection()
    ...
    def handle_terraform_setup(self) -> bool:
        ...
        for key, value in valid_config.items():
            if 'api_' in key.lower() or 'secret' in key.lower():
                self.ui.print_success(f"  {key}: ***{value[-4:] if len(value) > 4 else '***'}")
            else:
    ...
    def prompt_for_field(self, field: str, config: Dict[str, str], lab_selection: str = None) -> str:
        ...
        if field == 'prefix':
            ...
            default_value = existing_value if existing_value and existing_value in ['aws', 'azure'] else "azure"
            while True:
                if existing_value:
                    ...
                    if not response:
                        ...
                        # User wants to edit
                        value = self.ui.prompt("Cloud provider (aws/azure)", existing_value).lower()
                    else:
                        value = response.lower()
                else:
                    value = self.ui.prompt("Cloud provider (aws/azure)", default_value).lower()
            ...
            if existing_value and existing_value in available_regions:
                ...
                if not response:
                    ...
                    if response in available_regions:
                        self.config_manager.save_field_incrementally(field, response, lab_selection)
                        return response
                    else:
                        self.ui.print_error("Invalid region. Showing available options:")
            ...
            default_region = existing_value if existing_value in available_regions else available_regions[0]
            while True:
                value = self.ui.prompt("Select region (exact name or number)", default_region)
                if value.isdigit():
                    idx = int(value) - 1
                    if 0 <= idx < len(available_regions):
                        selected_region = available_regions[idx]
                        self.config_manager.save_field_incrementally(field, selected_region, lab_selection)
            ...
            if existing_value and self.config_manager.validate_config_value(field, existing_value, config):
                ...
                if not response:
                    ...
                    if self.config_manager.validate_config_value(field, response.strip(), config):
                        return response.strip()
                    else:
                        self.ui.print_error("Invalid API key format. Continuing with manual entry.")
            ...
            while attempts < max_attempts:
                value = self.ui.prompt("Enter your Confluent Cloud API Key", default_key)
            ...
            if existing_value and self.config_manager.validate_config_value(field, existing_value, config):
                ...
                if not response:
                    ...
                    if self.config_manager.validate_config_value(field, response.strip(), config):
                        ...
                        self.ui.print_error("Invalid API secret format. Continuing with manual entry.")

            attempts = 0
            max_attempts = 3
            default_secret = existing_value if existing_value else ""
            while attempts < max_attempts:
                value = self.ui.prompt("Enter your Confluent Cloud API Secret", default_secret)
            ...
            if existing_value and self.config_manager.validate_zapier_url(existing_value):
                ...
                if '/s/' in existing_value and '/sse' in existing_value:
                    ...
                    if start < end:
                        ...
                        masked_url = existing_value[:start] + masked_key + existing_value[end:]
                ...
                if not response:
                    ...
                    if self.config_manager.validate_zapier_url(response):
                        ...
                        self.ui.print_error("Invalid Zapier URL format. Continuing with manual entry.")
            ...
            while attempts < max_attempts:
                ...
                if attempts < max_attempts:
                    self.ui.print_error(f"❌ {max_attempts - attempts} attempts remaining.")
                else:
                    self.ui.print_error("❌ Maximum attempts reached. Setup cannot continue without Zapier endpoint.")
            return ""
        ...
        # MongoDB credentials with detailed context
            ...
            if existing_value and existing_value.startswith('mongodb+srv://') and 'mongodb.net' in existing_value:
                # Mask part of the connection string for display
                masked_url = existing_value
                if '.mongodb.net' in existing_value:
                    parts = existing_value.split('.')
                    if len(parts) >= 2:
                        cluster_part = parts[0].split('//')[-1]  # Get cluster name after mongodb+srv://
                        if len(cluster_part) > 8:
                            masked_cluster = cluster_part[:4] + '***' + cluster_part[-4:]
                            masked_url = existing_value.replace(cluster_part, masked_cluster)
                ...
                if not response:
                    ...
                    if response and response.startswith('mongodb+srv://') and 'mongodb.net' in response:
                        self.config_manager.save_field_incrementally(field, response, lab_selection)
                        return response
                    else:
                        self.ui.print_error("Invalid MongoDB connection string format. Continuing with manual entry.")

            self.ui.print_header("MongoDB Connection String", "Atlas cluster connection details")
            self.ui.print_info("📍 Location: MongoDB Atlas UI → Your Cluster → Connect → Connect your application -> Shell -> Copy from step 2")
            ...
            if existing_value:
                ...
                if not response:
                    ...
                    if response.strip():
                        self.config_manager.save_field_incrementally(field, response.strip(), lab_selection)
                        return response.strip()
            ...
            if existing_value:
                masked_password = '***' + existing_value[-3:] if len(existing_value) > 3 else '***'
