...
def main():
    print("Starting Azure Lab2 Document Publisher...")

    # Get Confluent Cloud credentials and endpoints from terraform
    try:
        kafka_bootstrap_endpoint = get_core_terraform_output(
            "confluent_kafka_cluster_bootstrap_endpoint"
        )
        schema_registry_endpoint = get_core_terraform_output(
            "confluent_schema_registry_rest_endpoint"
        )
        environment_name = get_core_terraform_output(
        ...
        sr_api_secret = get_core_terraform_output(
    ...
    try:
        producer = AvroProducer(
            producer_config, default_value_schema=json.dumps(value_schema)
        )
    ...
    for md_file in md_files:
        try:
            ...
            message_value = {"document_id": document_id, "document_text": content}

            # Produce message
            producer.produce(topic=topic_name, key=document_id, value=message_value)
