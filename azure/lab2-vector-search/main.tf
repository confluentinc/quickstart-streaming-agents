...
resource "confluent_flink_statement" "documents_vectordb_create_table" {
  ...
  statement = "CREATE TABLE IF NOT EXISTS documents_vectordb_lab2 ( document_id STRING, chunk STRING, embedding ARRAY<FLOAT> ) WITH ( 'connector' = 'mongodb', 'mongodb.connection' = 'mongodb-connection', 'mongodb.database' = '${var.MONGODB_DATABASE}', 'mongodb.collection' = 'documents_lab2', 'mongodb.index' = '${var.MONGODB_INDEX_NAME}', 'mongodb.embedding_column' = 'embedding', 'mongodb.numCandidates' = '500' );"
...
resource "null_resource" "generate_flink_sql_summary" {
  ...
  provisioner "local-exec" {
    command     = "python ${path.module}/../../scripts/common/generate_lab_flink_summary.py lab2 azure ${path.module} mongodb_connection_string='${var.mongodb_connection_string}' mongodb_username='${var.mongodb_username}' mongodb_password='${var.mongodb_password}' || true"
