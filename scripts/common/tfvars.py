...
def generate_lab1_tfvars_content(mcp_token: str, mcp_endpoint: str) -> str:
    """
    ...
    Args:
        mcp_token: MCP server authentication token
...
def write_tfvars_for_deployment(
    ...
    if "lab1-tool-calling" in envs_to_deploy:
        mcp_token = get_credential_value(creds, "mcp_token")
        mcp_endpoint = get_credential_value(creds, "mcp_endpoint")
        if mcp_token and mcp_endpoint:
            lab1_tfvars_path = root / "terraform" / "lab1-tool-calling" / "terraform.tfvars"
            content = generate_lab1_tfvars_content(mcp_token, mcp_endpoint)
