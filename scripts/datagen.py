...
Usage:
    uv run datagen aws                    # Generate data for AWS environment
    uv run datagen azure                  # Generate data for Azure environment
    uv run datagen terraform              # Use terraform directory setup
    uv run datagen --dry-run              # Validate setup without running
    uv run datagen aws --duration 300     # Run for 5 minutes
...
def run_shadowtraffic(
    ...
    if not env_file:
        logger.warning("⚠️  No ShadowTraffic environment file found")
        logger.warning("   Expected one of: free-trial-license-docker.env, shadowtraffic.env, .env")
        logger.warning("   ShadowTraffic may use trial limits")
...
Examples:
  ...
  uv run datagen terraform              # Use terraform directory setup
  uv run datagen aws --duration 300     # Run for 5 minutes
  uv run datagen --dry-run              # Validate setup only
