...
Usage:
    uv run publish-queries aws "SELECT * FROM documents LIMIT 10"
    uv run publish-queries azure "SHOW TABLES"
    uv run publish-queries "SELECT COUNT(*) FROM documents"  # Auto-detect
    uv run publish-queries aws --interactive                 # Interactive mode

Traditional Python:
    pip install -e .
    publish-queries aws "your query here"
"""
...
Examples:
  ...
  uv run publish-queries azure "SHOW TABLES"
  uv run publish-queries "SELECT COUNT(*) FROM documents"  # Auto-detect
  uv run publish-queries aws --interactive
  uv run publish-queries aws "SELECT * FROM docs" --topic custom_queries
